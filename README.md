...
| `template` | `TemplateFillSpec` | **模板填值(P3)** 加载完后按 spec 填一道再渲染。`placeholders`(`{{key}}` / `{{a.b.c}}` 替换)+ `anchors[]`(`startCell + rows` 按位铺数据,对象数组按 `columns` 顺序,`trimUnused` 默认 `true` 自动清理"模板预留但 JSON 没填"的空白行 —— 防边框看起来像幽灵数据);可配 `onProgress` + `signal` |
| `templateFile` | `ExcelSource` | **渲染模板(P3 进阶)** 一份 .xlsx 当渲染基。同时给 `:workbook` 时:加载模板 → 把 `:workbook` 数据按 `:template` spec 填进去渲染。工具栏内置 `template` 项可在运行时切换 / 导入 / 清除,无需重新挂载组件 |
| `templateName` | `string` | 模板显示名(标题栏 `· 模板: xxx` 后缀);不给则取运行时 File.name。`:fileName` 同步:JSON 源未给名时默认 "JSON 数据" |
| `exportProgress` | `boolean`(默认 `true`)| **内置导出进度遮罩(P1.5)** 调 `viewer.downloadPdf` / `downloadImage` / `downloadXlsx` / `print` / `applyTemplate` / 选区图片批量转换 时,壳自动建 `AbortController` + 接 `onProgress` → 显示居中模态(stage 标签 + 进度条 + 取消)。**关闭**用 `false`(纯回调走 `opts.onProgress`/`signal`);**自渲染**用 `#export-progress` 插槽(Vue)/ `renderExportProgress` (React) |
...
所有导出方法(PNG / PDF / XLSX / Print)+ `applyTemplate` + 选区图片批量互转 统一接:
```ts
...
**不传任何参数**调 `viewer.downloadPdf()` / `applyTemplate(spec)` 等异步方法,壳自动建 `AbortController` + 接 `onProgress` → 显示**居中模态**(stage 标签 + 进度条 + 取消按钮)。用户传入的 `onProgress`/`signal` **仍正常链回调**(并存,不冲突)。
...
覆盖矩阵:**导出**(PDF / PNG / XLSX / Print)、**applyTemplate**(P3)、**选区图片批量互转**(P2,壳侧 1.2.0 起返 `Promise<number>` 以接遮罩)。**不包含**:文件解析(parsing 有独立的顶栏进度条,与本遮罩分开)、`copySelection` / `setStyle` 等瞬时操作。
...
```
- **内置 id**:`find`(查找)、`filter`(切换自动筛选 —— 文件没设也能点出下拉)、`clear-filter`(清除筛选,无筛选时禁用)、`copy`(复制选区)、`wrap-text`(自动换行 toggle,WPS 风格,需 `editable`)、`image-tools`(图片工具 ▾:选区/整表/整列 浮动 ⇄ 嵌入互转,需 `editable`)、`template`(模板 ▾:默认 / 导入 .xlsx / 清除;P3 进阶)、`freeze`(冻结/取消)、`zoom`(缩放下拉)、`export`(导出/打印下拉)、`'separator'`/`'|'`(分隔线);`sort` 规划中。
