...
| WPS 内嵌图 | `getCellImages()` · `setCellImageFit('fill'\|'contain'\|'cover')` · `convertImageToCellAuto(imgIdx)`(就近) · `convertAllImagesToCells(col?)`(整表/整列) · `convertCellImageToFloat(row,col,size?)`(嵌入→浮动) · `convertImageToCell(imgIdx,row,col)`(显式目标) |
...
**长任务进度 + 取消(P1)** —— 所有导出方法(PNG / PDF / XLSX / Print)统一接 `onProgress?: (p: ExportProgress) => void` + `signal?: AbortSignal`:
```ts
const ctrl = new AbortController()
try {
  await viewer.value.downloadPdf({
    target: 'all',
    onProgress: (p) => console.log(p.stage, p.ratio, p.label), // 'render'/'compose'/'paginate'/'write'/'zip'
    signal: ctrl.signal,
  })
} catch (e) {
...
ctrl.abort()
```
导出全链路在调度点 `await yieldToEvent()` 让出 UI(防假死)+ 调度前 `checkAborted(signal)`(立刻中断)。`ExcelJS.writeBuffer` / `jsPDF` 内部仍是黑盒(`zip`/`write` 阶段),那两段无法细分,但用户从启动到完成已经有持续可视进度。
...
```
- **内置 id**:`find`(查找)、`filter`(切换自动筛选 —— 文件没设也能点出下拉)、`clear-filter`(清除筛选,无筛选时禁用)、`copy`(复制选区)、`wrap-text`(自动换行 toggle,WPS 风格,需 `editable`)、`freeze`(冻结/取消)、`zoom`(缩放下拉)、`export`(导出/打印下拉)、`'separator'`/`'|'`(分隔线);`sort` 规划中。
