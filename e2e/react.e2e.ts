...
test.describe('React 壳 e2e(与 Vue 共用 core: 渲染/选区/查找/数据/导出)', () => {
  test('解析→canvas 渲染→点选更新状态栏→Ctrl+F 查找→数据 API→PNG 导出', async ({ page }) => {
    ...
    // 4) PNG 导出走 core/WorkbookExporter,真实下载:开工具栏「导出」菜单 → 点「导出为图片 (PNG)」
    await page.getByRole('button', { name: '导出', exact: true }).click()
    const [dl] = await Promise.all([
      page.waitForEvent('download', { timeout: 45_000 }), // html2canvas 光栅化在 headless 较慢,放宽超时
      page.getByText('导出为图片 (PNG)').click(),
    ])
    expect(dl.suggestedFilename()).toMatch(/\.png$/)
  })

  test('跨框架插件: overlay DOM 徽标显示 + 插件工具栏按钮生效', async ({ page }) => {
    await loadReactSample(page)

    // 插件 overlay 返回的 DOM 徽标(🎯,锚在 B3)挂到了网格层
    const badge = page.locator('.plugin-badge')
    await expect(badge).toBeVisible()
    await expect(badge).toHaveText('🎯')

    // 插件贡献的工具栏按钮:点击 → setSelection(A2) → 公式栏地址变 A2
    await page.getByRole('button', { name: '🔌 插件按钮' }).click()
    await expect(page.locator('.rxl-formula-bar .addr')).toHaveText('A2')
  })
})
