...
- **测试是回归网**:改动后 `npm run typecheck` + `npm test`(单测)+ `npm run test:e2e`(Playwright 真浏览器)+ `npm run build` 必须全绿。当前基线 **368 单测 + 159 e2e(Vue 3 / React / Vue 2 三壳覆盖;Vue 2 e2e 跑独立 5302 dev server,见 `e2e/vue2-smoke.e2e.ts`)**。
...
- 仍未做:真正 workspace 多包拆分;条件格式可编辑(现只读渲染);Vue 2 子入口体积优化 (现 423 KB, 共享 chunk 需自定义 rollup);rebuild 导出模式不搬运原文件只读透视表(仅 overlay);大文件编辑性能
