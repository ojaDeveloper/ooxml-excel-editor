...
- **测试是回归网**:改动后 `npm run typecheck` + `npm test`(单测)+ `npm run test:e2e`(Playwright 真浏览器)+ `npm run build` 必须全绿。当前基线 **330 单测 + 124 e2e(Vue 3 + React 双覆盖,Vue 2 e2e 后续补)**。
...
- 仍未做:真正 workspace 多包拆分;Vue 2 e2e 覆盖;Vue 2 子入口体积优化 (现 423 KB, 共享 chunk 需自定义 rollup);rebuild 导出模式不搬运原文件只读透视表(仅 overlay);大文件编辑性能
